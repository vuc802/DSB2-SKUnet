...
SUNNYBROOK_ROOT_PATH = BASE_DIR + "Sunnybrook_data\\"
...
def convert_to_grayscale_with_increase_brightness_fast(im,threshhold=1):
    """
    ...
    out[out <= (out_mean*threshhold)] = out_mean #Energy Alignment
...
def load_contour(contour, img_path,save_dir):
    ...
    new_fname = contour.case + filename
    cv2.imwrite(SAVE_DIR_img + new_fname.replace(".dcm", ".png"),img)
    cv2.imwrite(SAVE_DIR_gt +new_fname.replace(".dcm", "_gt.png"),lab)
...
def change_countour(countour_path):
    ...
    gt = (np.where(gt>200,0,255)).astype('uint8')
    return gt
...
def load_manual_data(fdir):
    ...
    for i in test:
        filename=(((im_list[i]).split("\\"))[-1]).replace(".jpg","")
        img=cv2.imread(im_list[i],cv2.IMREAD_GRAYSCALE)
        g=change_countour(g_list[i])
        img=convert_to_grayscale_with_increase_brightness_fast(img,1)
        g=cv2.resize(g,(176,176))
        img=cv2.resize(img,(176,176))
        cv2.imwrite(BASE_DIR + 'test\\gt\\'+filename+'_gt.png',g)
        cv2.imwrite(BASE_DIR + 'test\\img\\'+filename+'.png',img)
    ...
    for i in train:
        ...
        g=cv2.resize(g,(176,176))
        img=cv2.resize(img,(176,176))
        cv2.imwrite(BASE_DIR + 'train\\gt\\'+filename+'_gt.png',g)
        cv2.imwrite(BASE_DIR + 'train\\img\\'+filename+'.png',img)
...
def load_manual_julia_data(img,gt):
    ...
    for i in test:
        ...
        g=cv2.resize(g,(176,176))
        cv2.imwrite(BASE_DIR + 'test\\gt\\'+filename+'_gt.png',g)
        cv2.imwrite(rBASE_DIR + 'test\\img\\'+filename+'.png',im)
    for i in train:
        filename=(os.path.basename(img[i])).replace(".png", "")
        im=convert_to_grayscale_with_increase_brightness_fast(cv2.imread(img[i],cv2.IMREAD_GRAYSCALE),1)
        im=cv2.resize(im,(176,176))
        ...
        g=cv2.resize(g,(176,176))
        cv2.imwrite(BASE_DIR + 'train\\gt\\'+filename+'_gt.png',g)
        cv2.imwrite(BASE_DIR + 'train\\img\\'+filename+'.png',im)
    for i in val:
        filename=(os.path.basename(img[i])).replace(".png", "")
        im=convert_to_grayscale_with_increase_brightness_fast(cv2.imread(img[i],cv2.IMREAD_GRAYSCALE),1)
        im=cv2.resize(im,(176,176))
        ...
        g = (np.where(g>0,255,0)).astype('uint8')
        g=cv2.resize(g,(176,176))
        cv2.imwrite(BASE_DIR + 'validate\\gt\\'+filename+'_gt.png',g)
        cv2.imwrite(BASE_DIR + 'validate\\img\\'+filename+'.png',im)

def load_stacom_data(img,gt):
    indice=np.arange(len(img))
    np.random.shuffle(indice)
    split_indice=len(img)*0.8
    split_indice1=len(img)*0.9
